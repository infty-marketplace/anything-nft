<template>
  <div>
    ...
    <b-modal ref="reg-modal" title="Register" @ok="handleRegister">
      <div class="mb-4">
        We noticed that this is your first time connecting to our platform, so
      ...
      <b-form-input
        class="mb-4"
        v-model="first_name"
        placeholder="Your creative first name here..."
      />
      <label>Last Name</label>
      <b-form-input class="mb-4" v-model="last_name" placeholder="Optional" />
    </b-modal>
    <b-toast id="no-wallet-toast" title="No Wallet Detected">
      <a href="https://portal.confluxnetwork.org/" target="_blank"
        >Please install it here.</a
      >
    </b-toast>
...
export default {
  ...
  methods: {
    ...
    toProfile() {
      ...
      if (
        ...
        path.split('profile/')[1] != this.$store.getters.getAddress
      ) {
  ...
  async created() {
    ...
    eventBus.$on('Navbar.connectWalletSuccess', async () => {
      axios
        .get(
          `${this.$store.getters.getApiUrl}/profile/${
            (await window.conflux.send('cfx_requestAccounts'))[0]
          }`
        )
        .then((res) => {
