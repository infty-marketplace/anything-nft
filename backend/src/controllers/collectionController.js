...
async function createNft(req, res) {
  ...
  const actualTokenId = cfxUtils.actualTokenId(
    req.body.address,
    uri,
    guessedTokenId
  );
...
async function transferOwnership(
  ...
  if (
    !buyer[`${collectionType}_ids`].includes(transactionDetails.collection_id)
  ) {
  ...
  if (transactionDetails.transaction_type === "purchase-album") {
    ...
    const index = [
      ...getNftOwners(collection),
      ...getNftFunders(collection),
    ].indexOf(transactionDetails.buyer);
    if (index >= 0) {
...
async function purchaseNtf(req, res) {
  ...
  await cfxUtils.transferOwnershipOnChain(
    nft.owner[0].address,
    body.buyer,
    tokenID
  );
